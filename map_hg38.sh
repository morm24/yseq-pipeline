...
cat ${YSEQID}_result_summary.txt | \
    ...
    sed "s|<SEQUENCED_BASES>|${SEQUENCED_BASES}|g" | \
...
##########################################################
	...
	#LOBSTR_PREFIX="/genomes/0/refseq/hg19/lobstr_v3.0.2_hg19_ref/lobSTR_"
	#LOBSTR_YSTR="/genomes/0/refseq/hg19/lobSTR_ystr_hg19.bed"
	#LOBSTR_ASTR="/genomes/0/refseq/hg19/lobSTR_codis_hg19.bed"
	#LOBSTR_OUTFILE="${YSEQID}_lobSTR"
	#
	#lobSTR \
	#  --index-prefix ${LOBSTR_PREFIX} \
	#  --files ${READS_1} \
	#  --fastq --gzip \
	#  --rg-sample ${YSEQID} --rg-lib ${YSEQID} \
	#  --fft-window-size 24 --fft-window-step 12 \
	#  --threads $NUM_THREADS \
	#  --out ${LOBSTR_OUTFILE}
	#
	#samtools sort -@ $NUM_THREADS -o ${LOBSTR_OUTFILE}.sorted.bam ${LOBSTR_OUTFILE}.aligned.bam
	#samtools index ${LOBSTR_OUTFILE}.sorted.bam
	#
	#
	#allelotype \
	#  --command classify \
	#  --bam ${LOBSTR_OUTFILE}.sorted.bam \
	#  --min-border 5 --min-bp-before-indel 7 --maximal-end-match 15 --min-read-end-match 5 \
	#  --noise_model ${LOBSTR_MODEL} \
	#  --out ${LOBSTR_OUTFILE} \
	#  --strinfo ${LOBSTR_STRINFO} \
	#  --index-prefix ${LOBSTR_PREFIX}
	#
	## Extract the YSTRs
	#intersectBed -a ${LOBSTR_OUTFILE}.vcf -b ${LOBSTR_YSTR} -wa -wb | \
	#   cut -f 1,2,10,14- | sed 's/:/\t/g' | cut -f 1,2,4,8,14- | sed 's/\//\t/' | \
	#   cut -f 4 --complement | awk '{print $0 "\t" $6+$4/$5}' > ${LOBSTR_OUTFILE}_YSTR.tsv
	#
	## Extract the autosomal STRs
	#intersectBed -a ${LOBSTR_OUTFILE}.vcf -b ${LOBSTR_ASTR} -wa -wb | \
	#   cut -f 1,2,10,14- | sed 's/:/\t/g' | cut -f 1,2,4,8,14- | sed 's/\//\t/' | \
	#   awk '{print $0 "\t" $7+$4/$6 "\t" $7+$5/$6}' > ${LOBSTR_OUTFILE}_aSTR.tsv
	#
